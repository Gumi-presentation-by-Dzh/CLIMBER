...
twlobj::twlobj(uint64_t areasize, int type, int climberenable, int randomenable, uint64_t climbershift, uint64_t climberthreshold){
        this->maxpagenums = areasize;
        this->attacktype = type;
        interinterval = 128;
        swapthreshold = 32;
        sprintf(endlifepath,"type%d_twlmm_wearrate_c.log",this->attacktype);
        logfile = fopen(endlifepath,"w");
        printf("maxpagenums:%lu\n",this->maxpagenums);
        sprintf(endlifepath,"type%d_twlmm_endlife_c.dat",this->attacktype);
...
uint64_t twlobj::getpairaddr(uint64_t*interswapcount,lifenode2*clifelist,uint64_t areasize,uint64_t addr_temp,uint64_t* intermaptable, uint64_t* reversemaptable,bool sourceaddr,bool iswrite){
    //global interinterval
        ...
        if (interswapcount[addr] >= interinterval){
            ...
            if ((sourceaddr0 ^ this->isswap[pairaddr0]) == 0){
                ...
                if(clifelist[this->sortedlist[intermaptable[addr]].addr].life<0){
                    this->returnstat = -1;
                }
            }
            else{
                ...
                if(clifelist[this->sortedlist[areasize - 1 - intermaptable[addr]].addr].life < 0){
                    this->returnstat = -1;
                }
            } 
    ...
    uint64_t twlobj::access(uint64_t addr_temp1,uint64_t addr_temp2,bool iswrite){
        ...
        if (isdoswap == 1){
            ...
            bool swaptemp = 0;
            if(sourceaddr == 1){
                swaptemp = this->swaparbiter(this->lifelist[nowaddr2].life,this->lifelist[addr].life);
            }
            else{
            ...
            if ((swaptemp ^ this->isswap[pairaddr]) == 1){
                ...
                if(this->lifelist[this->sortedlist[this->maxpagenums - 1 - this->pairlist[addr]].addr].life<0){
                    this->returnstat = -1;
                }
