...
NVMainMemory::NVMainMemory(std::string& nvmainTechIni, std::string& outputFile, std::string& traceName, uint32_t capacityMB, uint64_t _minLatency, uint32_t _domain, const g_string& _name , std::string fetcher_name) {
	...
	evlisthead = (struct evlist*) malloc(sizeof(struct evlist));
	evlisttail = (struct evlist*) malloc(sizeof(struct evlist));
	evlisthead->next = evlisttail;
	evlisthead->child = NULL;
	evlisthead->back = NULL;
	evlisttail->next = NULL;
	evlisttail->child = NULL;
	evlisttail->back = evlisthead;
	evlisthead->childtail = evlisthead->child;
	evlistp = NULL;
	isdo = 0;
    ...
    //string climbertype;
    //getline(climbercfg,climbertype);
    climberp = new climberif(capacityMB<<8,(zinfo->climbtype));
...
inline NVMainAccEvent* NVMainMemory::pageread(Address pageno, MemReq& req, uint64_t respCycle) {
	uint64_t addr = 0;
	NVMainAccEvent* memEv = NULL;
	for(int i =0; i < (1<<(zinfo->page_shift - lineBits)); i++){
		addr = (((pageno<<zinfo->page_shift)>>lineBits) + i)<<lineBits;
    	memEv = new (zinfo->eventRecorders[req.srcId])
    	 NVMainAccEvent(this, 0, addr, domain);
    	memEv->setMinStartCycle(req.cycle);
    	if(evlistp == NULL){
    		evlistp = (struct evlist*) malloc(sizeof(struct evlist));
    		evlistp->ev = memEv;
    		evlistp->isend = 0;
    		evlistp->isissued = 0;
    		evlistp->ev->backp = (void*)evlistp;
			evlistp->ev->ischild = 0;
    		evlistp->child = (struct evlistnode*) malloc(sizeof(struct evlistnode));
    		evlistp->childtail = (struct evlistnode*) malloc(sizeof(struct evlistnode));
    		evlistp->child->next = evlistp->childtail;
    		evlistp->childtail->back = evlistp->child;
    		evlistp->next = evlisttail;
    		evlistp->back =evlisttail->back;
    		evlisttail->back->next = evlistp;
    		evlisttail->back = evlistp;
    		evlisttail = evlistp->next;
    		if((req.type == GETX || req.type == GETS) && isdo==0 ){
    		...
    		struct evlistnode* newlistnode = (struct evlistnode*) malloc(sizeof(struct evlistnode));
    		newlistnode->back = evlistp->childtail->back;
    		newlistnode->next = evlistp->childtail;
    		newlistnode->backp = (void*)evlistp;
    		evlistp->childtail->back->next = newlistnode;
    		evlistp->childtail->back = newlistnode;
    		newlistnode->ev = memEv;
    		//memEv->backp = (void*)evlistp;
    		if((req.type == GETX || req.type == GETS) && isdo==0 ){
    			TimingRecord tr = {addr, req.cycle, respCycle, req.type, memEv, memEv};
    	        zinfo->eventRecorders[req.srcId]->pushRecord(tr);
    	        NVMainAccEvent* memEvtemp = newlistnode->ev;
    	        newlistnode->ev = evlistp->ev;
    	        evlistp->ev = memEvtemp;
    	        evlistp->ev->backp = (void*)evlistp;
				evlistp->ev->ischild = 0;
    	        isdo = 1;
    		}
    		newlistnode->ev->backp = (void*)newlistnode;
			newlistnode->ev->ischild = 1;
    	}
	}
	return memEv;
}
inline NVMainAccEvent* NVMainMemory::pagewrite(Address pageno, MemReq& req, uint64_t respCycle) {
	uint64_t addr = 0;
	NVMainAccEvent* memEv = NULL;
	for(int i =0; i < (1<<(zinfo->page_shift - lineBits)); i++){
		addr = (((pageno<<zinfo->page_shift)>>lineBits) + i)<<lineBits;
		memEv = new (zinfo->eventRecorders[req.srcId])
		 NVMainAccEvent(this, 1, addr, domain);
		memEv->setMinStartCycle(req.cycle);	
    	if(evlistp == NULL){
    		evlistp = (struct evlist*) malloc(sizeof(struct evlist));
    		evlistp->ev = memEv;
    		...
    		//newlistnode->backp = (void*)evlistp;
    		evlisttail = evlistp->next;
    		if((req.type == PUTX || req.type == PUTS) && isdo==0 ){
    			TimingRecord tr = {addr, req.cycle, respCycle, req.type, memEv, memEv};
    	        zinfo->eventRecorders[req.srcId]->pushRecord(tr);
    	        isdo = 1;
    		}
...
inline NVMainAccEvent* NVMainMemory::push_to_main_memory( MemReq& req, uint64_t respCycle )
{
    ...
    Address pageno = addr >> zinfo->page_shift;
    //Address pageaddr = (~((-1 >> zinfo->page_shift) << zinfo->page_shift))&addr;
    uint64_t * returnaddr;
	returnaddr = climberp->access(pageno, req.type == PUTX && zinfo->isclimb);
	...
	if(returnaddr[0] == 0){
    	...
    	evlisttail->back = evlistp;
    	evlisttail = evlistp->next;
    	req.cycle += returnaddr[4];
    	respCycle += returnaddr[4];
	...
	else if (returnaddr[0] == 1){
  		pageread(returnaddr[1],req,respCycle);
  		pageread(returnaddr[2],req,respCycle);
  		pagewrite(returnaddr[1],req,respCycle);
  		zinfo->swap2 += 1;
  		memEv = pagewrite(returnaddr[2],req,respCycle);
	}
	else if (returnaddr[0] == 2){
		pageread(returnaddr[3],req,respCycle);
   		pageread(returnaddr[1],req,respCycle);
   		pagewrite(returnaddr[3],req,respCycle);
   		pageread(returnaddr[2],req,respCycle);
   		pagewrite(returnaddr[2],req,respCycle);
   		zinfo->swap3 += 1;
   		memEv = pagewrite(returnaddr[1],req,respCycle);
	}
	else{
		pageread(returnaddr[1],req,respCycle);
		if(returnaddr[5] != 0){
...
uint64_t NVMainMemory::access(MemReq& req) {
    ...
    if (zinfo->isclimb == 1){
    	respCycle += 5;///5 for index in maptable
    }
...
/*****cswhb*/
uint64_t NVMainMemory::enqueuechild(NVMainAccEvent* ev, uint64_t cycle){
    ...
    while(!nvmainPtr->IsIssuable(request, NULL)){
    	request->startCycle = curCycle;
	    request->arrivalCycle = curCycle;
    	
    	//std::cout<<"reissue2 request:"<<request->address.GetPhysicalAddress()<<std::endl;
        //info("[%s] [Enqueue] Update NVMain %lu cycles", getName(), curCycle - updateCycle);
        if(curCycle >= updateCycle){
        	curCycle = curCycle + 1;
            nvmainGlobalEventQueue->Cycle(curCycle - updateCycle);
...
//    }
    ...
    //inflightRequests2.push_back(std::make_pair(request, std::make_pair(ev, cycle + minLatency)));
    ev->hold();
...
void NVMainMemory::enqueue(NVMainAccEvent* ev, uint64_t cycle) {
	//std::cout<<"enqueue of zsim"<<std::endl;
    /****cswhb***/
    struct evlist*p =(struct evlist*)(ev->backp);
    struct evlistnode*p2 =NULL;
    uint64_t cyclenow = cycle+1;
    bool isswap = 0;
    curCycle = cycle;
    //for(p=evlisthead->next;p!=evlisttail;p=p->next){
    	//if(p->ev == ev){
    		
    		//p->child->next = p->childtail;
    		//p->childtail->back = p->child;
    		//p->back->next = p->next;
    		//p->next->back = p->back;
    		//break;
    	//}
    ...
    if (!nvmainPtr->IsIssuable(request, NULL)) {
        //info("[%s] %s access to %lx requeued. Curent cycle %ld, requeue cycle %ld", getName(), ev->isWrite()? "Write" : "Read", ev->getAddr(), cycle, cycle+1);
		//std::cout<<"reissue1 request:"<<request->address.GetPhysicalAddress()<<std::endl;
        //ev->requeue(cycle+10);
        ev->requeue(updateCycle);
        delete request;
        return;
    }
    //info("[%s] [Enqueue] Address %lx curCycle %lu, cycle %lu, updateCycle %lu, inflight requests %ld", getName(), ev->getAddr(), curCycle, cycle, updateCycle, inflightRequests.size());
	...
	if(p->isissued == 0){
		for(p2 = p->child->next;p2!=p->childtail;p2=p2->next){
		    //	assert_msg(p2->backp == p,"p2:%d,p:%d",p2->backp,p);
   		    //assert_msg(p2->ev->backp == p2,"ev:%d,p2%d",p2->ev->backp,p2);
   			cyclenow += 1;
   			isswap = 1;
    	}
    	//p->isissued = 1;
    ...
    //}
    // Add this request to list of inflight requests
    request->inflightp = (void*) (inflightRequests.push_back(std::make_pair(request, std::make_pair(ev, cyclenow - 1 + minLatency))));
    ev->hold();
    ...
    if (!nextSchedEvent) {
        ...
        nextSchedEvent->enqueue(cyclenow - 1 + minLatency);
        nextSchedRequest = request;
    }
    /****cswhb***/
    assert(zinfo->climbtype == 1 || zinfo->climbtype == 2 || zinfo->climbtype == 3);
    if(zinfo->climbtype == 1){
    	cyclenow = cycle+15;
    }
    else if (zinfo->climbtype == 2){
    	cyclenow = cycle+15;
    }
    else if (zinfo->climbtype == 3){
    	cyclenow = cycle+15;
    }
    else{
    	printf("undefine climbertype\n");
    	cyclenow = cycle + 1;
    }
	if(p->isissued == 0){
		for(p2 = p->child->next;p2!=p->childtail;p2=p2->next){
		    //	assert_msg(p2->backp == p,"p2:%d,p:%d",p2->backp,p);
   		    //assert_msg(p2->ev->backp == p2,"ev:%d,p2%d",p2->ev->backp,p2);
   			cyclenow = enqueuechild(p2->ev, cyclenow);
   			isswap = 1;
    	}
    	p->isissued = 1;
	}
...
bool NVMainMemory::totalcomplete(struct evlist* evlist1) {
	request_complete++;
	NVM::NVMainRequest *creq = (NVM::NVMainRequest *)(evlist1->request);
	evlist1->back->next = evlist1->next;
	evlist1->next->back = evlist1->back;
	free(evlist1);
    ...
    NVMainAccEvent* ev = ((struct inflightnode*)(creq->inflightp))->data.second.first;
	...
	inflightRequests.erase((struct inflightnode*)(creq->inflightp));
...
bool NVMainMemory::RequestComplete(NVM::NVMainRequest *creq) {
    ...
    //auto it = inflightRequests.begin();
    //auto it2 = inflightRequests2.begin();
    //for (; it2 != inflightRequests2.end(); ++it2) {
    //	if (it2->first == creq) break;
    //}
    //if(it2!=inflightRequests2.end()){
	NVMainAccEvent* ev =(NVMainAccEvent*) creq->bkev;
	if(ev->ischild == 1){
    	futex_lock(&return_lock);
    	struct evlistnode* evnode1 = (struct evlistnode*)(ev->backp);
        struct evlist* evlist1 =(struct evlist*)(evnode1->backp);
        evnode1->back->next = evnode1->next;
        evnode1->next->back = evnode1->back;
		//delete evnode1->ev;
		//evnode->
        free(evnode1);

        //it2->second.first->release();
        //inflightRequests2.erase(it2);
        //printf("%d\n",evlist1);
        if (evlist1->child->next ==evlist1->childtail){
        	if(evlist1->isend == 1){
        		if (creq == nextSchedRequest)
                    nextSchedRequest = NULL;

                futex_unlock(&return_lock);
        		delete creq;
        		
        		return totalcomplete(evlist1);
        	}
        }
        else{
        	if (creq == nextSchedRequest)
    ...
    //}
    //assert(it != inflightRequests.end());
    struct evlist* evlist1 =(struct evlist*)(((struct inflightnode*)(creq->inflightp))->data.second.first->backp);
    if (evlist1->child->next != evlist1->childtail){
        evlist1->request = (void*) creq;
        evlist1->isend = 1;
        return true;
    }
    evlist1->back->next = evlist1->next;
    evlist1->next->back = evlist1->back;
    free(evlist1);
    ev = ((struct inflightnode*)(creq->inflightp))->data.second.first;
    // Note that curCycle is up to date because we are advancing cycle by cycle in tick while
    // we are waiting for a request completion.
    //uint64_t lat = curCycle+1 - ev->sCycle;
    uint64_t lat = curCycle+1 - creq->startCycle;
