...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
    ...
    int isclimb = config.get<uint32_t>("sys.mem.isclimb", 0);
    string climbtype = config.get<const char*>("sys.mem.climbtype", "climber");
    printf("isclimb:%d",isclimb);
    zinfo->isclimb = isclimb;
    zinfo->swap2 = 0;
    zinfo->swap3 = 0;
    if(climbtype == "climber"){
        printf("climbertype:1\n");
        zinfo->climbtype = 1;
    }
    else if(climbtype == "twl"){
        printf("climbertype:2\n");
        zinfo->climbtype = 2;
    }
    else if(climbtype == "bwl"){
        printf("climbertype:3\n");
        zinfo->climbtype = 3;
    }
    else{
        printf("climbertype:0\n");
        zinfo->climbtype = 0;
    }
