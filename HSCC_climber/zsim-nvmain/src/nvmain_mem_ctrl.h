...
struct evlistnode{
	struct evlistnode* back;
	NVMainAccEvent* ev;
	void*backp;
	struct evlistnode* next;
};
struct evlist{
	NVMainAccEvent* ev;
	struct evlistnode* child;
	struct evlist*back;
	struct evlist*next;
	bool isend; 
	bool isissued;
	void*request;
	//struct evlist*tail;
	struct evlistnode* childtail;
};
struct inflightnode{
	std::pair<NVM::NVMainRequest*, std::pair<NVMainAccEvent*, uint64_t>> data;
	struct inflightnode*next;
	struct inflightnode*back;
};
class inflightlist{
	public:
	uint64_t totalsize;
	struct inflightnode*head;
	struct inflightnode*tail;
	inflightlist(){
		head = (struct inflightnode*) malloc(sizeof(struct inflightnode));
		tail = (struct inflightnode*) malloc(sizeof(struct inflightnode));
		head->next = tail;
		head->back = NULL;
		tail->back = head;
		tail->next = NULL;
	...
	std::pair<NVM::NVMainRequest*, std::pair<NVMainAccEvent*, uint64_t>> front(){
		return head->next->data;
	}
	struct inflightnode* push_back(std::pair<NVM::NVMainRequest*, std::pair<NVMainAccEvent*, uint64_t>> indata){
		struct inflightnode*newnode = (struct inflightnode*) malloc(sizeof(struct inflightnode));
		newnode->data = indata;
		newnode->next = tail;
	...
	void erase(struct inflightnode* eranode){
		eranode->back->next = eranode->next;
		eranode->next->back = eranode->back;
		free(eranode);
		this->totalsize--;
	}
	bool empty(){
...
class NVMainMemory : public MemObject, public NVM::NVMObject { //one NVMain controller
    private:
		...
		inflightlist inflightRequests;
        //std::vector<std::pair<NVM::NVMainRequest*, std::pair<NVMainAccEvent*, uint64_t>>> inflightRequests2;
	...
	public:
		...
		uint64_t issued_num;
		climberif* climberp;
		struct evlist* evlisthead;
		struct evlist* evlisttail;
		struct evlist* evlistp;
		bool isdo;
		//struce evlist* evlisttail;
    public:
        ...
        uint64_t access(MemReq& req,int stat);
        inline NVMainAccEvent* pageread(Address pageno, MemReq& req, uint64_t startcycle);
        inline NVMainAccEvent* pagewrite(Address pageno, MemReq& req, uint64_t startcycle);

        // Event-driven simulation (phase 2)
        uint64_t tick(uint64_t cycle);
        uint64_t enqueuechild(NVMainAccEvent* ev, uint64_t cycle);
        bool totalcomplete(struct evlist* evlist1);
